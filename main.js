...
function renderMetadata(m, c) {
  const nowPlayingContainer = document.getElementById('nowPlayingSection');
  const topCastContainer = document.getElementById('topCastSection');
  const fallbackMeta = document.getElementById('metaContent'); 
  ...
  const topCastHtml = `
    <div class="card border-0 rounded-4 shadow-sm" style="background: var(--surface); border: 1px solid var(--border) !important;">
      <div class="card-body">
  ...
  if (nowPlayingContainer && topCastContainer) {
      nowPlayingContainer.innerHTML = nowPlayingHtml;
      topCastContainer.innerHTML = topCastHtml;
  } else if (fallbackMeta) {
      fallbackMeta.innerHTML = nowPlayingHtml + '<div class="mt-4"></div>' + topCastHtml;
  }
...
try {
  ...
  if (m.success === false) throw new Error(m.status_message || "TMDB API Error");
  
  currentImdbId = ext?.imdb_id || null;
  currentPosterPath = m?.poster_path || null;
  currentTitle = m?.title || m?.name || 'Unknown';
  ...
  if (t === 'tv') {
    if (tvSelectors) {
        tvSelectors.classList.remove('d-none');
    ...
    if (tvSelectors) {
        tvSelectors.classList.add('d-none');
        tvSelectors.innerHTML = '';
    }
  }
  ...
  const errContainer = document.getElementById('nowPlayingSection') || document.getElementById('playerContainer') || document.getElementById('metaContent');
  if (errContainer) {
      errContainer.innerHTML = `<div class="alert alert-danger rounded-4 m-3 shadow-sm"><i class="fa fa-triangle-exclamation me-2"></i>Failed to load details. Please check your connection or try again.</div>`;
  }
  toggleLoader(false);
}
}
...
try {
  ...
  if(episodeList) {
      episodeList.innerHTML = '<div class="col-12 text-center text-danger">Failed to load episodes</div>';
  }
}
}
...
try {
    ...
    const soundtrackContainer = document.getElementById('soundtrackSection');
    const galleryContainer = document.getElementById('gallerySection');
    const moreLikeThisContainer = document.getElementById('moreLikeThisSection');
    const fallbackMeta = document.getElementById('metaContent');

    const soundtrackHtml = `
      <div class="card border-0 rounded-4 shadow-sm" style="background: var(--surface); border: 1px solid var(--border) !important;">
        <div class="card-body">
          ...
          <div class="song-list-container" id="soundtrackList"></div>
        </div>
      </div>
    `;
    ...
    if (soundtrackContainer && galleryContainer && moreLikeThisContainer) {
        soundtrackContainer.innerHTML = soundtrackHtml;
        galleryContainer.innerHTML = galleryHtml;
        moreLikeThisContainer.innerHTML = moreLikeThisHtml;
    } else if (fallbackMeta) {
        const comboContainer = document.createElement('div');
        comboContainer.className = "mt-4";
        comboContainer.innerHTML = soundtrackHtml + galleryHtml + moreLikeThisHtml;
        fallbackMeta.appendChild(comboContainer);
    }
